  ...
  showQuestComplete(quest) {
    ...
    modal.innerHTML = `
          <div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl max-w-md w-full mx-4 p-6 text-center shadow-2xl animate-bounce">
            ...
            <p class="text-white text-lg mb-2">${quest.name}</p>
            <p class="text-white/90 text-sm mb-4">${quest.description}</p>
            <div class="bg-white/20 rounded-lg p-3 mb-4">
              <p class="text-white">Reward: +${quest.reward} XP</p>
            </div>
            <button id="closeQuestComplete" class="px-6 py-2 bg-white text-green-600 rounded-lg font-semibold hover:bg-gray-100 transition">
              Continue Journey!
            </button>
          </div>
        `;
  ...
  showPathUnlock(pathId) {
    ...
    modal.innerHTML = `
          <div class="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl max-w-md w-full mx-4 p-6 text-center shadow-2xl animate-bounce">
            ...
            <p class="text-white text-xl mb-2">${path.name}</p>
            <p class="text-white/90 text-sm mb-4">${path.description}</p>
            <div class="bg-white/20 rounded-lg p-3 mb-4">
              <p class="text-white">Rewards: ${path.rewards.points} points + "${path.rewards.badge}" badge</p>
            </div>
            <button id="closePathUnlock" class="px-6 py-2 bg-white text-purple-600 rounded-lg font-semibold hover:bg-gray-100 transition">
              Let's Go!
            </button>
          </div>
        `;
  ...
  showLevelUp(level) {
    ...
    modal.innerHTML = `
          <div class="bg-gradient-to-br from-yellow-500 to-orange-500 rounded-2xl max-w-md w-full mx-4 p-6 text-center shadow-2xl animate-bounce">
            ...
            <p class="text-white text-2xl mb-2">Level ${level}</p>
            <p class="text-white/90 text-sm">You're getting better! Keep going!</p>
            <button id="closeLevelUp" class="mt-4 px-6 py-2 bg-white text-orange-600 rounded-lg font-semibold hover:bg-gray-100 transition">
              Awesome!
            </button>
          </div>
        `;
  ...
  showLearningPaths() {
    ...
    modal.innerHTML = `
          <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-5xl w-full mx-4 p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4 sticky top-0 bg-white dark:bg-gray-800 py-2">
              <h3 class="text-2xl font-bold text-gray-800 dark:text-white">
                <i class="fas fa-road mr-2 text-green-500"></i>Learning Paths
              </h3>
              <button id="closePathsPanel" class="text-gray-500 hover:text-gray-700 dark:text-gray-400">
                <i class="fas fa-times text-2xl"></i>
              </button>
            </div>
            
            <!-- Player Stats -->
            <div class="mb-6 p-4 bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl text-white">
              <div class="flex justify-between items-center">
                <div>
                  <p class="text-sm opacity-90">Level ${this.userProgress.currentLevel}</p>
                  <p class="text-2xl font-bold">${this.userProgress.totalXP} XP</p>
                </div>
                <div>
                  <p class="text-sm opacity-90">Quests Complete</p>
                  <p class="text-2xl font-bold">${completedCount}/${totalQuests}</p>
                </div>
                <div class="text-right">
                  <p class="text-sm opacity-90">Current Path</p>
                  <p class="text-lg font-bold">${currentPath.name}</p>
                </div>
              </div>
              <div class="mt-2 w-full bg-white/30 rounded-full h-2">
                <div class="bg-white h-2 rounded-full" style="width: ${(this.userProgress.totalXP % 100)}%"></div>
              </div>
              <p class="text-xs opacity-75 mt-1">${100 - (this.userProgress.totalXP % 100)} XP to next level</p>
            </div>
      ...
      return `
                    <div class="p-3 rounded-xl ${isActive ? 'bg-green-100 dark:bg-green-900/30 border-2 border-green-500' : isUnlocked ? 'bg-gray-100 dark:bg-gray-700' : 'bg-gray-200 dark:bg-gray-800 opacity-50'}">
                      ...
                      <p class="text-xs text-gray-500">${path.description}</p>
                      ${!isUnlocked ? `<p class="text-xs text-gray-400 mt-1">🔒 Reach Level ${path.requiredLevel}</p>` : ''}
      ...
      return `
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      <div class="flex-1">
                        <div class="flex items-center gap-2">
                          ...
                          ${isCompleted ? '<span class="text-xs text-green-500">✓ Complete</span>' : ''}
                        </div>
                        <p class="text-xs text-gray-500">${quest.description}</p>
                      </div>
                      <div class="text-right">
                        <p class="text-sm font-bold text-yellow-500">+${quest.reward} XP</p>
                        ${!isCompleted ? '<p class="text-xs text-gray-400">Not started</p>' : ''}
                      </div>
                    </div>
                  </div>
                `;
            ...
            <!-- Skill Tree -->
            <h4 class="font-semibold mb-3">Skill Tree</h4>
            <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
              ${Object.entries(this.userProgress.skillTree).map(([skill, data]) => `
                <div class="p-3 text-center rounded-lg ${data.unlocked ? 'bg-purple-100 dark:bg-purple-900/30' : 'bg-gray-200 dark:bg-gray-700 opacity-50'}">
                  <div class="text-2xl mb-1">${this.getSkillIcon(skill)}</div>
                  <p class="font-semibold text-sm capitalize">${skill}</p>
                  <p class="text-xs">Level ${data.level}</p>
                  <div class="w-full bg-gray-300 rounded-full h-1 mt-1">
                    <div class="bg-purple-500 h-1 rounded-full" style="width: ${data.xp}%"></div>
                  </div>
                </div>
              `).join('')}
            </div>
            
            <button id="closePathsBtn" class="w-full px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition">
              Close
            </button>
          </div>
        `;
  ...
  showPremiumModal() {
    ...
    modal.innerHTML = `
          <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-4xl w-full mx-4 p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4 sticky top-0 bg-white dark:bg-gray-800 py-2">
              <h3 class="text-2xl font-bold text-gray-800 dark:text-white">
                <i class="fas fa-crown mr-2 text-yellow-500"></i>Premium Membership
              </h3>
              <button id="closePremiumModal" class="text-gray-500 hover:text-gray-700 dark:text-gray-400">
                <i class="fas fa-times text-2xl"></i>
              </button>
            </div>
            
            ${isPremium ? `
              <div class="mb-6 p-4 bg-green-100 dark:bg-green-900/30 rounded-xl text-center">
                <p class="text-lg font-semibold text-green-700 dark:text-green-300">✨ You are a Premium ${currentTier.name} member! ✨</p>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Thank you for supporting VelocityType</p>
                ${this.userSubscription.endDate ? `<p class="text-xs mt-2">Valid until: ${new Date(this.userSubscription.endDate).toLocaleDateString()}</p>` : ''}
              </div>
            ` : ''}
      ...
      return `
                  <div class="p-4 rounded-xl ${isCurrent ? 'bg-gradient-to-br from-yellow-400 to-orange-500 text-white' : 'bg-gray-100 dark:bg-gray-700'}">
                    ...
                    <p class="text-xs opacity-75">or $${tier.price}/year</p>
                    <ul class="mt-3 space-y-1 text-xs">
                      ${tier.features.slice(0, 3).map(f => `<li>✓ ${f}</li>`).join('')}
                      ${tier.features.length > 3 ? `<li class="text-xs opacity-75">+${tier.features.length - 3} more features</li>` : ''}
                    </ul>
                    ${!isCurrent && this.userSubscription.tier !== 'free' ? `
                      <button class="upgrade-btn mt-3 w-full py-1 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm transition" data-tier="${tier.id}">
                        ${this.userSubscription.tier === 'free' ? 'Upgrade' : 'Switch to ' + tier.name}
                      </button>
                    ` : ''}
                    ${!isCurrent && this.userSubscription.tier === 'free' ? `
                      <button class="upgrade-btn mt-3 w-full py-1 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm transition" data-tier="${tier.id}">
            ...
            <h4 class="font-semibold mb-3">Feature Comparison</h4>
            <div class="overflow-x-auto mb-6">
              <table class="w-full text-sm">
                <thead class="bg-gray-100 dark:bg-gray-700">
                  <tr>
                    <th class="p-2 text-left">Feature</th>
                    <th class="p-2 text-center">Free</th>
                    <th class="p-2 text-center">Pro</th>
                    <th class="p-2 text-center">Team</th>
                    <th class="p-2 text-center">Enterprise</th>
                  </tr>
                </thead>
                <tbody>
                  <tr class="border-b"><td class="p-2">Unlimited test history</td>${this.renderCheckmarks(false, true, true, true)}</tr>
                  <tr class="border-b"><td class="p-2">Advanced analytics</td>${this.renderCheckmarks(false, true, true, true)}</tr>
                  <tr class="border-b"><td class="p-2">Export data</td>${this.renderCheckmarks(false, true, true, true)}</tr>
                  <tr class="border-b"><td class="p-2">Multiplayer racing</td>${this.renderCheckmarks(false, true, true, true)}</tr>
                  <tr class="border-b"><td class="p-2">Daily challenges</td>${this.renderCheckmarks(false, true, true, true)}</tr>
                  <tr class="border-b"><td class="p-2">Classroom dashboard</td>${this.renderCheckmarks(false, false, true, true)}</tr>
                  <tr class="border-b"><td class="p-2">API access</td>${this.renderCheckmarks(false, false, true, true)}</tr>
                  <tr class="border-b"><td class="p-2">Custom branding</td>${this.renderCheckmarks(false, false, false, true)}</tr>
                  <tr class="border-b"><td class="p-2">Priority support</td>${this.renderCheckmarks(false, true, true, true)}</tr>
                </tbody>
              </table>
            </div>
            
            <button id="closePremiumBtn" class="w-full px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition">
              Close
            </button>
          </div>
        `;
  ...
  showPaymentModal(tierId) {
    ...
    modal.innerHTML = `
          <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full mx-4 p-6 shadow-2xl">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-xl font-bold text-gray-800 dark:text-white">
                <i class="fas fa-credit-card mr-2 text-blue-500"></i>Upgrade to ${tier.name}
              </h3>
              <button id="closePaymentModal" class="text-gray-500 hover:text-gray-700 dark:text-gray-400">
                <i class="fas fa-times text-xl"></i>
              </button>
            </div>
            
            <div class="mb-4 p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
              <p class="text-center text-2xl font-bold">$${tier.priceMonthly}<span class="text-sm">/month</span></p>
              <p class="text-center text-sm text-gray-500">or $${tier.price}/year (save 17%)</p>
            </div>
            
            <div class="space-y-3 mb-4">
              <input type="text" placeholder="Cardholder Name" class="w-full p-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700">
              <input type="text" placeholder="Card Number" class="w-full p-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700">
              <div class="flex gap-2">
                <input type="text" placeholder="MM/YY" class="flex-1 p-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700">
                <input type="text" placeholder="CVC" class="flex-1 p-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700">
              </div>
            </div>
            
            <div class="flex gap-3">
              <button id="subscribeBtn" class="flex-1 px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition">
                <i class="fas fa-lock"></i> Subscribe Now
              </button>
              <button id="cancelPaymentBtn" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition">
                Cancel
              </button>
            </div>
  ...
  showWelcomeToPremium(tierId) {
    ...
    modal.innerHTML = `
          <div class="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-2xl max-w-md w-full mx-4 p-8 text-center shadow-2xl animate-bounce">
            ...
            <p class="text-white/90 mb-4">Thank you for upgrading! You now have access to all premium features.</p>
            <div class="bg-white/20 rounded-lg p-3 mb-4">
              <p class="text-white text-sm">✨ New features unlocked ✨</p>
              <ul class="text-white/90 text-xs mt-2 space-y-1">
                ${tier.features.slice(0, 5).map(f => `<li>✓ ${f}</li>`).join('')}
              </ul>
            </div>
            <button id="closeWelcome" class="px-6 py-2 bg-white text-orange-600 rounded-lg font-semibold hover:bg-gray-100 transition">
              Start Exploring!
            </button>
          </div>
        `;
  ...
  addAdBanner() {
    ...
    adBanner.innerHTML = `
          <div class="flex items-center gap-3">
            ...
            <button id="upgradeFromAd" class="ml-2 px-3 py-1 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg text-xs transition">
              Upgrade Now
            </button>
          </div>
        `;
